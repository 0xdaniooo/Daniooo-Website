...
softwareIcons.set("eleventy", "https://img.shields.io/badge/Eleventy-00A776?style=for-the-badge&logo=eleventy&logoColor=white");
...
export function handleHashRouting(onMatchFound: (targetId: string, openModal: boolean) => void) 
{
    const processHash = () => {
        ...
        // Differentiate type based on suffix
        if (rawHash.endsWith("-history")) 
        {
            targetId = rawHash.replace("-history", "");
            openModal = true;
        }
        ...
        if (element) 
        {
            ...
            onMatchFound(targetId, openModal);
        }
    };
